...
public:
    ...
    std::unique_ptr<numbers_generator> clone() const {
        return std::unique_ptr<numbers_generator>(this->cloneWrapped());
    };
private:
    virtual numbers_generator * cloneWrapped() const = 0;
};
...
private:
    virtual fixed_generator * cloneWrapped() const override {
...
private:
    ...
    std::mt19937_64 rng;
    virtual mersenne_twister_generator * cloneWrapped() const override {
...
private:
    ...
    std::minstd_rand rng;
    virtual minstd_generator * cloneWrapped() const override {
